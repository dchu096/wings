...
func Configure(m *wserver.Manager, client remote.Client) *gin.Engine {
	...
	router.GET("/api/servers/:server/devtools/ws", middleware.ServerExists(), getServerDevtoolsWebsocket)
	...
	server.Use(middleware.RequireAuthorization(), middleware.ServerExists())
	{
		...
		devtoolsGroup := server.Group("/devtools")
		{
			devtoolsGroup.GET("", getServerDevtools)
			devtoolsGroup.POST("", postServerDevtools)
			devtoolsGroup.POST("/power", postServerDevtoolsPower)
			devtoolsGroup.GET("/ssh-hook", getServerDevtoolsSSHHook)
			devtoolsGroup.POST("/ssh", postServerDevtoolsSSH)
			devtoolsGroup.DELETE("/ssh", deleteServerDevtoolsSSH)
		}
