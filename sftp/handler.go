...
// quotaWriterAt maps filesystem disk-space errors to SFTP quota status codes and
// blocks writes while the server is installing, transferring, or restoring.
...
func (h *Handler) Filewrite(request *sftp.Request) (io.WriterAt, error) {
	...
	// Force a fresh disk usage lookup — a stale cache lets oversized uploads pass the gate.
