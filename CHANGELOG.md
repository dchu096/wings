...
## axis-1.0.0
### Security (ported from upstream Wings ≤ v1.13.3)
* **GHSA-hfvm-879c-xgmm / GHSA-8j54-xcwx-597p** — SFTP streaming writes enforce disk quotas (`quotaFile` + fresh open-time check); SFTP denied during install/transfer/restore
* **GHSA-5mgr-9frx-2rh7** — overflow-safe `Quota.CanFit` / `Add`; archive decompress size accumulation cannot wrap
* **GHSA-ghrq-5wpp-hxx5** — reject SFTP Setstat packets that request extended attributes (handshake DoS / unbounded alloc)
* **GHSA-rhq6-9rgh-v45c** — `fchmodat` uses `AT_SYMLINK_NOFOLLOW` so chmod cannot escape via symlink
* **GHSA-pfvc-3p5h-x7h6** — egg `{{config.*}}` templating no longer exposes full daemon secrets
* **GHSA-4g8v-75pm-x7f6** — Docker registry auth matches by parsed domain/path (not string prefix)
* **GHSA-77pg-7gfq-9hjr** — backup identifiers must be canonical UUIDs (blocks path traversal)
* **GHSA-7w9q-4q32-wfj8** — S3 backup restore downloads block SSRF to private/loopback hosts
* **GHSA-q6hh-gp44-4hcm** — refuse to parse oversized egg configuration files (OOM guard)
* **CVE-2026-54593 / GHSA-8r6w-3qq5-4p4r** — JWT purpose scoping for upload/download tokens (`scope` claim)
* **CVE-2026-39882** — bump OpenTelemetry OTLP HTTP exporter to ≥ v1.43 (`otlptracehttp` v1.46.0)
* **CVE-2026-56855 / GO-2026-6355+** — bump `golang.org/x/crypto` to v0.57.0 (SSH-related)

### Dependencies
* Go toolchain **1.26.7** (required by patched `x/crypto` + OpenTelemetry)
* `github.com/docker/docker` → v28.5.2
* `github.com/gin-gonic/gin` → v1.12.0
* `github.com/klauspost/compress` → v1.20.0
* `github.com/buger/jsonparser` → v1.1.2
* `github.com/quic-go/quic-go` → v0.59.1 (transitive via gin)
* `golang.org/x/crypto` → v0.57.0 (and related `x/*` bumps)

### Added
* Locked-down DevTools sidecar (`docs/devtools-sidecar.md`, contract `devtools.v1`)
* Axis branding (binary `axis`, version `axis-1.0.0`)

### Notes
* Upload/download JWTs must include a `scope` claim (`file-upload`, `file-download`, `backup-download`) as issued by Panel ≥ 1.12.3 / Atlas equivalents.
* After upgrading, rotate node daemon tokens if the config-templating leak may have been exploited.
