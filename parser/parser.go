...
// templatableConfig is the only subset of daemon configuration exposed to egg
// {{config.*}} placeholders. Marshalling the full config leaked tokens and other
// secrets (GHSA-pfvc-3p5h-x7h6).
...
func (f *ConfigurationFile) Parse(file ufs.File) error {
	...
	// this guards the daemon against being OOM'd by an oversized config.
