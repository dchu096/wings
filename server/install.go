...
func (s *Server) internalInstall() error {
	// Avoid file races with an active developer sidecar during install.
	if mgr := s.DevTools(); mgr != nil {
		_ = mgr.Stop(s.Context())
	}
...
// IsInProtectedState reports whether the server is installing, transferring, or
// restoring — states where tenant filesystem mutations (including SFTP) must be denied.
...
func (ip *InstallationProcess) pullInstallationImage() error {
	...
	if _, c := config.Get().Docker.RegistryCredentialsForImage(ip.Script.ContainerImage); c != nil {
		log.WithField("registry_image", ip.Script.ContainerImage).Debug("using authentication for registry")
		registryAuth = c
	}
