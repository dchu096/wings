...
type Server struct {
	...
	// Locked-down developer sidecar (see docs/devtools-sidecar.md).
	devtoolsOnce sync.Once
	devtoolsMgr  *devtools.Manager
}
...
func (s *Server) CleanupForDestroy() {
	...
	s.powerLock.Destroy()
	if mgr := s.DevTools(); mgr != nil {
		_ = mgr.Destroy(context.Background())
	}
}
...
// DevTools returns the locked-down developer sidecar manager for this server.
func (s *Server) DevTools() *devtools.Manager {
	s.devtoolsOnce.Do(func() {
		if s.fs == nil {
			return
		}
		mgr, err := devtools.New(s.ID(), s.fs.Path(), s.Log().WithField("subsystem", "devtools"))
		if err != nil {
			s.Log().WithField("error", err).Error("failed to initialize devtools manager")
			return
		}
		s.devtoolsMgr = mgr
	})
	return s.devtoolsMgr
}

// SyncDevTools applies panel-synced DevTools settings to the sidecar manager.
func (s *Server) SyncDevTools() {
	mgr := s.DevTools()
	if mgr == nil {
		return
	}
	cfg := s.Config()
	mgr.SetImageOverride(cfg.DevToolsImage)
	if err := mgr.SetEnabled(s.Context(), cfg.DevToolsEnabled && !s.IsSuspended()); err != nil {
		s.Log().WithField("error", err).Warn("failed to reconcile devtools sidecar")
	}
}

// SetDevToolsEnabled updates the in-memory DevTools enable flag until the next panel sync.
func (s *Server) SetDevToolsEnabled(enabled bool) {
	s.cfg.SetDevToolsEnabled(enabled)
}
