...
type Configuration struct {
	...
	// DevToolsEnabled is synced from Atlas. When true, Axis may run the
	// locked-down developer sidecar for this server. Omitted/false on old panels.
	DevToolsEnabled bool `json:"devtools_enabled"`

	// DevToolsImage optionally overrides the node-default sidecar image.
	DevToolsImage string `json:"devtools_image,omitempty"`
}
...
func (c *Configuration) SetDevToolsEnabled(enabled bool) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.DevToolsEnabled = enabled
}
