...
// reserveDisk checks whether size bytes can fit, then increments the usage counter.
// Callers that stream writes (SFTP) use this before each Write/WriteAt.
...
// adjustDisk applies a delta to the cached usage under the filesystem lock.
